    ...
    <section id="top" className="relative pt-[clamp(3rem,2rem+4vw,7rem)] pb-[clamp(2rem,1rem+3.5vw,5rem)]">
      <div className="max-w-[1380px] mx-auto px-5 sm:px-6 lg:px-10">
        <div className="text-center max-w-5xl mx-auto pt-4 lg:pt-12">
          <div className="animate-slide-up">            <Link
            to={r.process}
          ...
          <h1 className="mt-4 lg:mt-6 headline fluid-h1 text-[var(--color-ink)] animate-slide-up delay-200">              {t.hero.titleA}
            <span className="italic font-light text-[var(--color-clay-deep)]"> {t.hero.titleB}</span>
          </h1>
          <p className="mt-5 lg:mt-7 max-w-2xl mx-auto fluid-body text-[var(--color-ink-soft)] animate-slide-up delay-300">              {t.hero.body}
          </p>
          <div className="mt-6 lg:mt-9 flex flex-wrap items-center justify-center gap-2 lg:gap-3 animate-slide-up delay-400">              <Link
            to={r.configurator}
        ...
        <dl className="mt-[clamp(2.5rem,1rem+5vw,7rem)] grid grid-cols-2 lg:grid-cols-4 gap-px bg-[var(--color-line)] border border-[var(--color-line)] rounded-md overflow-hidden animate-slide-up" style={{ animationDelay: '0.5s' }}>            {t.hero.stats.map(([n, body]) => (
